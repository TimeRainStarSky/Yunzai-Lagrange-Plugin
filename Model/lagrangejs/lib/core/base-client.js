...
exports.BaseClient = exports.QrcodeResult = exports.LogLevel = exports.ApiRejection = void 0;
...
(function (QrcodeResult) {
    QrcodeResult[QrcodeResult["Confirmed"] = 0] = "Confirmed";
    QrcodeResult[QrcodeResult["CodeExpired"] = 17] = "CodeExpired";
...
class BaseClient extends node_events_1.EventEmitter {
    constructor(uin, device, uid, p = device_1.Platform.Linux) {
        super();
        this.uin = uin;
        this[_a] = false;
        this[_b] = new ecdh_1.default('exchange', false);
        this[_c] = new ecdh_1.default('wtlogin', true);
        this[_d] = new network_1.default();
        this[_e] = new Map();
        this[_f] = false;
        this.sig = {
            password: constants_1.BUF0,
            seq: (0, crypto_1.randomBytes)(4).readUInt32BE() & 0xfff,
            tgtgt: constants_1.BUF16,
            tgt: constants_1.BUF0,
            d2: constants_1.BUF0,
            d2Key: constants_1.BUF16,
            qrSig: constants_1.BUF0,
            signApiAddr: 'http://127.0.0.1:7458/api/sign',
            exchangeKey: constants_1.BUF0,
            keySig: constants_1.BUF0,
            cookies: '',
            captchaUrl: '',
            aid: '',
            ticket: '',
            randStr: '',
            unusualSig: constants_1.BUF0,
            tempPwd: constants_1.BUF0,
        };
        this.interval = 10;
        ...
        this.platform = p;
        this.uid = uid;
        this.appInfo = (0, device_1.getAppInfo)(p);
        ...
        this[NET].on('connect2', () => {
            this.statistics.remoteIp = this[NET].remoteAddress;
            this.statistics.remotePort = this[NET].remotePort;
        ...
        (0, constants_1.lock)(this, 'appInfo');
        ...
        (0, constants_1.hide)(this, 'ssoInterval');
    }
    ...
    async keyExchange() {
        ...
        const gcmCalc1 = (0, constants_1.aesGcmEncrypt)(plain1, this[ECDH256].shareKey);
        const ts = (0, constants_1.timestamp)();
        const plain2 = new writer_1.default()
            .writeBytes(this[ECDH256].publicKey)
            .writeU32(1) // type
            .writeBytes(gcmCalc1)
            .writeU32(0) // const
            .writeU32(ts);
        const hash = (0, constants_1.sha256)(plain2.read());
        const gcmCalc2 = (0, constants_1.aesGcmEncrypt)(hash, Buffer.from('e2733bf403149913cbf80c7a95168bd4ca6935ee53cd39764beebe2e007e3aee', 'hex'));
        ...
        const decrypted = (0, constants_1.aesGcmDecrypt)(pbResp[1].toBuffer(), shareKey);
    ...
    async tokenLogin(token) {
        if (!this.sig.keySig.length || !this.sig.exchangeKey.length)
    ...
    async passwordLogin(md5) {
        if (!this.sig.keySig.length || !this.sig.exchangeKey.length)
            await this.keyExchange();
        this.sig.password = md5;
        const packet = buildNTLoginPacketBody.call(this, (0, tlv_1.getRawTlv)(this, 0x106, false, md5));
        ...
        if (resp === errors_1.LoginErrorCode.CaptchaVerify) {
            if (this.sig.captchaUrl && this.sig.aid)
                this.emit('internal.slider', this.sig.captchaUrl);
            else
                this.emit('internal.error.login', '[登陆失败]未知格式的验证码');
        }
        return resp;
    }
    async submitCaptcha(ticket, randStr) {
    ...
    [(_a = IS_ONLINE, _b = ECDH256, _c = ECDH192, _d = NET, _e = HANDLERS, _f = LOGIN_LOCK, FN_NEXT_SEQ)]() {
...
async function buildUniPacket(cmd, body, seq = 0) {
    seq = seq || this[FN_NEXT_SEQ]();
    this.emit('internal.verbose', `send:${cmd} seq:${seq}`, LogLevel.Debug);
    let head;
    const sign = await sign_1.getSign.call(this, cmd, seq, Buffer.from(body));
    if (sign) {
        head = pb.encode({
            15: (0, constants_1.trace)(),
            16: this.uid,
            24: {
    ...
    const ssoHeader = new writer_1.default()
        .writeU32(seq)
        ...
        .writeWithLength(cmd)
        .writeWithLength(constants_1.BUF0) // unknown
        ...
        .read();
    const ssoPacket = new writer_1.default().writeWithLength(ssoHeader).writeWithLength(body).read();
    const encrypted = tea.encrypt(ssoPacket, this.sig.d2Key);
    const service = new writer_1.default()
        ...
        .read();
    return new writer_1.default().writeWithLength(service).read();
}
function buildCode2dPacket(cmdid, body) {
...
function buildLoginPacket(cmd, body) {
    ...
    const writer = new writer_1.default()
        ...
        .writeU8(3)
        .read();
    const frame = new writer_1.default()
        .writeU8(2)
...
function buildNTLoginPacketBody(credential) {
    const proto = {
        1: {
            1: {
                1: this.uin.toString(),
    ...
    return pb.encode({
        1: this.sig.keySig,
        3: (0, constants_1.aesGcmEncrypt)(pb.encode(proto), this.sig.exchangeKey),
        4: 1,
    });
}
function decodeNTLoginResponse(encrypted) {
    const rawPb = pb.decode(encrypted);
    const decrypted = (0, constants_1.aesGcmDecrypt)(rawPb[3].toBuffer(), this.sig.exchangeKey);
...
async function decodeLoginResponse(payload) {
